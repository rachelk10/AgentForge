...
from app.schemas.skill import SkillCatalogResponse, SkillCreate, SkillResponse, SkillUpdate
...
async def create_skill(data: SkillCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_admin_user)) -> SkillResponse:
    return await SkillService(db).create(data, current_user.id)


@router.get("", response_model=list[SkillCatalogResponse])
async def list_skills(db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> list[SkillCatalogResponse]:
    return await SkillService(db).list_skills(include_disabled=current_user.is_admin)
...
async def get_skill(skill_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)) -> SkillResponse:
    return await SkillService(db).get_visible(skill_id, current_user)


@router.patch("/{skill_id}", response_model=SkillResponse)
async def update_skill(skill_id: uuid.UUID, data: SkillUpdate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_admin_user)) -> SkillResponse:
    return await SkillService(db).update(skill_id, data, None)


@router.delete("/{skill_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_skill(skill_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_admin_user)) -> None:
    await SkillService(db).delete(skill_id, None)


@router.post("/{skill_id}/enable", response_model=SkillResponse)
async def enable_skill(skill_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_admin_user)) -> SkillResponse:
    return await SkillService(db).update(skill_id, SkillUpdate(enabled=True), None)


@router.post("/{skill_id}/disable", response_model=SkillResponse)
async def disable_skill(skill_id: uuid.UUID, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_admin_user)) -> SkillResponse:
    return await SkillService(db).update(skill_id, SkillUpdate(enabled=False), None)
